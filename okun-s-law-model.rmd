...
## Load data
```{r}
Data <- read_csv("../input/okuns-law/Data.csv")
```
...
However, for this version of Data, I based on the several countries to know the special elements with them when we apply the same model. that is why I choose annual frequentists because we do have not the same volume of data for all countries that I had to choose it.
...
So, we have seen, the density function of our unemployment is not normally distribution, but it is mixte. we can not apply simple regression model in this step.
...
So, We can not apply linear regression model.
```{r}
...
```
We can ignore this right picture, because it does not have high affect of our model.
...
summary(model_6)
```
Okun Law in India situation exists, even if the destribution of Unemployment has usage in the right side.
...
that is means in Frensh economy, GDP Growth go up when Unemployment rate go up, that is not logical of course.
We can say Frensh economy has problem or does not depend by Labor fource for creating his wealth.
...
So right skewed or left skewed distribution does not affect on linear regression.
...
In the conclusion of this step, we have seen the importance of normal distribution, or approximatly normally distribution for build the linear model, So many case of countries we can not apply Okun Law model except `United Stated`, of course the origine of that model came from US economy, 
...
Now, I can not use Jags language inside R in Kaggle Colab, if Any one have any idea to install jags in Kaggle , I will apprechiated him or her for this helping, I think in the next step of this reseach I will focus only on my country to know how can estimate the coefficients of Okun Law with multimodal distribution.
I wish you are enjoy when you have read this notebook. it does not bad `isn't it????`
